...
function connectUser(ws, username) {
  ...
  getUserData(username, (userData) => {
    ...
    console.log(`🟢 Пользователь ${username} подключился (💰 ${userData.coins})`);
    ...
    db.all(
      ...
      (err, rows) => {
        if (err) {
          console.error('Ошибка получения переписок:', err);
          return;
        }
  ...
  ws.on('message', (data) => {
    try {
      ...
      switch (messageData.type) {
            ...
            db.run('UPDATE users SET coins = coins - ? WHERE username = ?', [gift.price, username], () => {
              ...
              db.run(
                ...
                (err) => {
                  ...
                  // Возвращаем монеты отправителю если он админ
                  if (senderData.isAdmin) {
                    db.run('UPDATE users SET coins = coins + ? WHERE username = ?', [gift.price, username]);
                  }
                  
                  // Добавляем подарок в список пользователя
        ...
        case 'give_admin':
          // Выдача админки (только для админов)
          getUserData(username, (adminData) => {
            if (!adminData || !adminData.isAdmin) {
              ws.send(JSON.stringify({ type: 'error', message: 'У вас нет прав для этого действия!' }));
              return;
            }
            ...
            db.run('UPDATE users SET isAdmin = 1 WHERE username = ?', [messageData.targetUsername], function(err) {
              if (this.changes > 0) {
                getUserData(messageData.targetUsername, (newAdmin) => {
                  ...
                  // Уведомляем нового админа если онлайн
  ...
  db.get('SELECT * FROM users WHERE username = ?', [username], (err, user) => {
    ...
    if (!user) {
      ...
      bcrypt.hash(password, 10, (err, hashedPassword) => {
        ...
        // Проверяем, является ли это Vortex
...
server.listen(PORT, () => {
  console.log(`🟣 Purple Messenger на порту ${PORT}`);
});
