...
function checkAchievements(username, userData, ws) {
      ...
      if (earned) {
        ...
        if (ws && ws.readyState === WebSocket.OPEN) {
          ws.send(JSON.stringify({
            ...
            achievement: { id: ach.id, name: ach.name, icon: ach.icon, description: ach.desc }
          }));
        }
      }
    });
...
wss.on('connection', (ws, req) => {
  ...
  db.get('SELECT * FROM users WHERE username = ?', [username], async (err, user) => {
    ...
    if (!user) {
      // Новый пользователь — ждём завершения INSERT
      ...
      await new Promise((resolve, reject) => {
        db.run('INSERT INTO users (username, password, isAdmin) VALUES (?, ?, ?)',
          [username, hashedPassword, isAdmin ? 1 : 0], function(err) {
            if (err) { console.error('Ошибка создания:', err); reject(err); }
            else { console.log(`🆕 Новый пользователь: ${username}`); resolve(); }
          });
      });
      
      // Получаем свежие данные после INSERT
      user = await new Promise((resolve) => {
        db.get('SELECT * FROM users WHERE username = ?', [username], (err, row) => {
          resolve(row || { username, isAdmin, coins: 0, level: 1, clicks: 0, avatar: '', achievements: [] });
        });
      });
    } else {
    ...
    // Список всех пользователей — теперь после INSERT
        ...
        switch (messageData.type) {
          ...
          case 'clicker':
            db.get('SELECT * FROM users WHERE username = ?', [username], (err, currentUser) => {
              ...
              checkAchievements(username, { clicks: currentUser.clicks + 1, coins: currentUser.coins + actualEarned, level: newLevel }, ws);
